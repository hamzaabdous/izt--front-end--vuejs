                  ...
                  <v-row>
                    ...
                    <v-col cols="6" md="6">
                      <v-select
                        ...
                        item-text="Label"
                        item-value="id"
                        v-model="editedItem.carrange_id"
                        label="Car range"
                        outlined
                      ></v-select>
                    </v-col>
          ...
          <v-dialog v-model="dialogDelete" max-width="600px">
            <v-card>
              ...
              <v-card-title class="text-h5"
                >Are you sure you want to delete this destination carranges
                ?</v-card-title
              >
              <v-card-actions>
                <v-spacer></v-spacer>
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: "Price (DH)", value: "Price", sortable: true },
      { text: "Depart", value: "Depart.Label", sortable: true },
      { text: "Destination", value: "destination.Label", sortable: true },
  ...
  methods: {
    initialize() {
      ...
      this.setDESTINATIONSAction().then(() => {
        this.destination = [...this.getDestinations];
      });
    ...
    editItem(item) {
      this.editedIndex = this.destinationcarranges.indexOf(item) + 1;
    ...
    deleteItemConfirm() {
      this.deleteDESTINATIONCARRANGEAction(this.editedItem).then(() => {
        this.destinationcarranges = this.destinationcarranges.filter((e) => {
    ...
    save() {
      if (this.editedIndex == -1) {
        this.addDESTINATIONCARRANGEAction(this.editedItem).then((equipment) => {
